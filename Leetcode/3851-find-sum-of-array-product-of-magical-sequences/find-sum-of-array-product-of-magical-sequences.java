class Solution {
    ...
    long dfs(int i, int mLeft, int carry, int kLeft, int[][] pows, int[][][][] memo) {
        ...
        if (ones + mLeft < kLeft)
            return 0; // agar baaki bits se k banana possible nahi toh return 0
        if (i == pows.length)
            return (mLeft == 0 && ones == kLeft) ? 1 : 0; // base case check
        if (memo[i][mLeft][carry][kLeft] != -1)
            return memo[i][mLeft][carry][kLeft]; // memo use karo
